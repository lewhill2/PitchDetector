...
const int FFTROWS = 128;  /// number of timesteps
const int FFTCOLS = 16;    /// outputs per timestep
const int BINCOLS = 128;
float fftTable[FFTROWS][FFTCOLS];
int nextSample = 0;

void storeSample(float data[], int n)
{
    for(int i = 0; i < FFTCOLS / BINCOLS; i++)
    {
        for(int j = 0; j < BINCOLS; j++)
            fftTable[nextSample][i] +=data[i*BINCOLS + j];  
        
    }
        
    nextSample++;
    if(nextSample > FFTROWS)
        nextSample = 0;
